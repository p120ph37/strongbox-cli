...
- `src/protocol/messages.ts` + `guards.ts` — outer-frame (`HandshakeFrame` | `EnvelopeFrame`) and inner-RPC (`RpcRequest` / `RpcResponse`) types. **Treat every concrete shape here as a hypothesis.** Runtime-validate anything coming off the wire through `guards.ts`; keep unknown fields as `unknown`, never `any`.
